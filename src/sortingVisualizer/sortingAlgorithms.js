...
export const shellSort = (array) => { 
    ...
    while (space > 0) {
        ...
        for (let i=0; i < space; i++) {
            ...
            animations.push([barIndexArray, compareColor, 'COMPARING'])
            ...
            for (let j = i+space; j <= last; j+=space) {
                ...
                while (index >= 0 && unsortedElement < array[index]) {
                    ...
                    // Changes color and values for bars as they shift
                    animations.push([index, array[index], index+space, array[index+space], secondCompareColor, 'SHIFTING'])
                    // Resets color of bars back to compareColor now that the swap is completed
                    animations.push([index, index+space, compareColor, 'DONE_SHIFTING'])
                    index -= space;
                }
